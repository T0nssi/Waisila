...
function generateId(): string {
  return 'pf' + Date.now().toString(36) + Math.random().toString(36).substring(2, 6);
}
...
export const GET: APIRoute = async ({ cookies }) => {
  ...
  if (fs.existsSync(PORTFOLIO_FILE)) {
    data = JSON.parse(fs.readFileSync(PORTFOLIO_FILE, 'utf-8'));
  }
...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  const newItem = {
    ...
    image: body.image || '',
    images: body.images || [],
    type: body.type || 'stone', // stone, factory, office, home
